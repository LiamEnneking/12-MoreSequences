...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(235, 100),  # All numbers are approximate.
                rg.Point(200, 135),
                rg.Point(165, 100),
                rg.Point(200, 65)]
...
def run_test_draw_points_on_circle():
    ...
    # Tests 4 and 5 (on the same window):
    title = 'Tests 4 and 5:  9 blue on deep pink;  56 green1 on unfilled.'
    window = rg.RoseWindow(440, 400, title)
    circle = rg.Circle(rg.Point(78, 234), 34)
    circle.fill_color = 'blue'
    draw_points_on_circle(window, circle, 9, 'purple')
    window.continue_on_mouse_click()

    circle = rg.Circle(rg.Point(243, 324), 45)
    draw_points_on_circle(window, circle, 56, 'tan')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  14 slices, thin (thickness=10) gray lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 180)
    circle.outline_thickness = 3
    circle.fill_color = 'green'
    pizza(window, circle, 15, 'gray', 10)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for i in range(-1, number_of_slices, 1):
        start = points[i]
        line = rg.Line(start, circle.center)
        line.color = color
...
def run_test_polygon():
    ...
    # Test 3:
    title = 'POLYGON test 3:  14 segments thin yellow.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 1
    polygon(window, circle, 14, 'yellow', 10)
    window.continue_on_mouse_click()
    ...
    # Test 4:
    title = 'POLYGON test 4:  10 segments thin red.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(150, 250), 125)
    circle.outline_thickness = 4
    polygon(window, circle, 10, 'red', 5)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 121, 23, 'brown', 90)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for i in range(-1, number_of_lines, 1):
        start = points[i]
        line = rg.Line(start, points[i-hops_to_next_point])
        line.color = color
        line.thickness = thickness
        line.arrow = 'last'
        line.attach_to(window)
